...
router.get("/api/client/get-all-categories", async (req, res) => {
  ...
  try {
    ...
    const categories = await db
      .collection("parent-category")
      .aggregate([
        // Accesses the parent-category collection in the database.
        {
          $lookup: {
            // Joins the sub-category collection with parent-category.
            from: "sub-category",
        ...
        },
        {
          ...
          }, // Deconstructs the sub_categories array into individual objects. If sub_categories is empty, preserveNullAndEmptyArrays: true keeps the parent document in the result.
        },
        {
          $lookup: {
        ...
        },
        {
          $group: {
            ...
            status: { $first: "$status" },
            sub_categories: {
              // Collects sub_categories as an array, containing each sub-category and its nested sub_sub_categories.
              $push: {
        ...
        },
        {
          $project: {
            ...
            par_cat_name: 1,
            status: 1, // Includes par_cat_name and sub_categories.
            sub_categories: {
              $filter: {
                // Filters out sub_categories where sub_cat_id is null.
